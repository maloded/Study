...
const routing = {
    ...
    '/user/age': () => user.age,
    '/hello': { hello: 'world', andArray: [1, 2, 3, 4, 5, 6, 7, 8] },
    '/api/method1': (req, res, callback) => {
        console.log(`${req.url} ${res.statusCode}`);
        callback({ status: res.statusCode });
    },
    '/api/method2': req => ({ user, url: req.url, cookie: req.headers.cookie }),
};
